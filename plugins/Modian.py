...
from plugins import modian


listen_group = [615830027, 480408585]
...
def onQQMessage(bot, contact, member, content):
    ...
    if contact.ctype != 'group':
        return
    if int(contact.qq) not in listen_group:
        return

    if member.qq == bot.conf.qq:
        if '--admin' in content:
            if 'version' in content:
                bot.SendTo(contact, 'Version: {0}.{1}.{2}'.format(0, 0, 1))
            if 'command' in content:
                ret = modian.auto_response(content)
                if ret is not None:
